/* Service worker — offline support for localpdf.
...
*/
const CACHE = "localpdf-offline-v1";
...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil((async () => {
    ...
    // Same-origin libs loaded via JS (not a src/href in the HTML, so not parsed above)
    // — precache them so render/export tools work offline on the FIRST use. Chiefly the
    // pdf.js WORKER (set via workerSrc in app.js): without it, the first offline render
    // threw a misleading "file may be corrupt" error, contradicting the "100% Offline"
    // promise. (The large OCR WASM still caches at runtime — too big to block install on.)
    try {
      const extraLibs = ["lib/pdf.worker.min.js", "lib/pdf.min.js", "lib/pdf-lib.min.js"];
